...
**Версия**: 6.5.0 (пайплайн для Kotlin + Spring backend)
...
DevTeam устанавливается в два этапа: сначала основные компоненты через `qwen extension`, затем дополнительные через `install.sh`.

#### Шаг 1: Основные компоненты (`qwen extension`)
...
Устанавливает:
- ✅ Slash commands (`/devteam:build`, `/devteam:analyze`, etc.)
- ✅ Skills (35 штук)
- ✅ Agents (25 штук)
- ✅ MCP servers (graphfocus, atlassian)
- ✅ QWEN.md контекст

#### Шаг 2: Хуки и состояние (`install.sh`)

```bash
# User-level: в ~/.qwen/ — глобально для всех проектов
bash install.sh

# Project-level: в <project>/.qwen/ — изолирует от других проектов
bash install.sh /path/to/your/project
```

Устанавливает:
- ✅ Lifecycle hooks (PreToolUse, PostToolUse, Stop, SessionStart, SessionEnd, etc.)
- ✅ State initialization (`.devteam/state/`)
- ✅ Sentinel file (idempotency check)

#### Почему два шага?

`qwen extension` не поддерживает lifecycle hooks и state initialization — для них остаётся `install.sh`.

| Компонент | `qwen extensions install .` | `install.sh` |
|-----------|:---:|:---:|
| Slash commands | ✅ | ❌ |
| Skills | ✅ | ❌ |
| Agents | ✅ | ❌ |
...
- `install.sh /path/to/project` → устанавливает в `<project>/.qwen/`
  (`.devteam/` создаётся рядом с `.qwen/` — в корне проекта)
- `install.sh` (внутри git-репозитория) → auto-detect: `<cwd>/.qwen/`
- `install.sh` (вне git) → user-level: `~/.qwen/`
...
2. Проверьте, что оба шага установки выполнены:
   - Шаг 1: `qwen extensions install .` (устанавливает agents, commands, skills)
   - Шаг 2: `bash install.sh` (устанавливает hooks и state)
3. Определите, куда устанавливали:
   - Project-level: `ls <project>/.qwen/.devteam-installed`
   - User-level: `ls ~/.qwen/.devteam-installed`
4. Проверьте hooks: `cat <target>/settings.json | grep -A 5 hooks`
5. Если hooks отсутствуют, перезапустите install.sh:
   `bash install.sh` (user-level) или `bash install.sh /path/to/project` (project-level)
